...
## CWD five-fold protocol / FINE reporting boundary (2026-08-10)

- **CWD implementation:** the existing per-fold estimator, global objective, optimizer, and
  final-test-only evaluation are unchanged. The reproduction runner now constructs all five
  folds with one split seed, isolates every fold directory, resumes an interrupted fold at an
  epoch boundary, skips completed folds without rewriting them, and writes
  `aggregate_metrics.json` only after all five folds complete. Its mean/std use each fold's
  final held-out test accuracy and no test result participates in training or selection.
- **CWD reproduction status:** `protocol_ready`, not `protocol_completed`. The repository has
  not run the full 200-epoch budget for all five folds under this orchestrator. Earlier
  single-fold training evidence remains historical evidence, not a five-fold aggregate.
- **FINE implementation:** complete fixed-budget workflow with final-test-only evaluation.
  Historical single-run evidence and the historical ledger value `last-10 = 67.50% ± 0.20%`
  exist, but the current artifact set has neither per-epoch test metrics nor the last ten model
  snapshots. Therefore that last-k value is not reconstructible from the current runner, and
  multi-seed numerical reproduction remains unavailable.
- **FINE blocker / next separately approved step:** persist the final `k=10` model snapshots,
  finish training without test access, then evaluate those snapshots after training and report
  last-k mean/std. This was not implemented in the current phase.
