...
def run_volminnet_experiment(
    ...
    *,
    ...
    if resume is not None:
        ...
        saved_state = (resume_payload.get("algorithm") or {}).get("volminnet_state", {})
        if bool(saved_state.get("completed")) and int(
            saved_state.get("completed_epochs", -1)
        ) >= epochs:
            _validate_artifacts(run_dir, resume_payload.get("artifact_hashes", {}))
            return run_dir
