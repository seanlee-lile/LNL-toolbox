...
def _evaluate(
    ...
    for batch in loader:
        ...
        image_condition = images if images.ndim == 4 and images.shape[1] == 3 else None
...
def _run_legacy_dld_experiment(
    config: Mapping[str, Any],
    ...
    *,
    ...
    if session is not None:
        for name in ("dld_features.npz", "dld_pre_correction.npz", "dld_noise_source.json"):
            candidate = run_dir / name
            if candidate.is_file():
                session.register_artifact(name, candidate)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_number, batch in enumerate(data.train_loader):
            ...
            predicted_residual, predicted_noise, residual, sampled_noise, _ = model.forward_t(
                y_input, targets, features, timesteps, images=image_condition
            )
        ...
        if session is not None:
            ...
            atomic_save(checkpoint_payload, run_dir / "last.pt")
...
def run_dld_experiment(
    ...
    *,
    ...
    settings = config.get("dld", {}) if isinstance(config, Mapping) else {}
    if isinstance(settings, Mapping) and any(key in settings for key in ["fidelity","feature_extractor"]):
        return _run_dld_paper_workflow(dict(config), output_dir=output_dir, resume=resume)
    return _run_legacy_dld_experiment(config, output_dir=output_dir, resume=resume, context=context)
