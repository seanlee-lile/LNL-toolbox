const state = { blocks: [], recipe: { schema_version: 1, name: 'scratch_recipe', description: '', steps: [] }, selected: null };
...
function drawSteps(steps = state.recipe.steps, parent = $('steps')) {
  ...
  steps.forEach((step, index) => {
    ...
    if (info && info.kind !== 'action') { const nested = document.createElement('div'); nested.className = 'nested'; drawSteps(step.steps || (step.steps = []), nested); node.appendChild(nested); }
...
$('check').onclick = async () => { try { await api('/api/validate', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(payload()) }); $('output').textContent = '检查通过'; $('output').className = 'ok'; } catch (error) { $('output').textContent = error.message; $('output').className = 'error'; } };
$('save').onclick = async () => { try { const result = await api('/api/save', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(payload()) }); $('output').textContent = `已保存: ${result.path}`; } catch (error) { $('output').textContent = error.message; } };
$('run').onclick = async () => { try { const result = await api('/api/run', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(payload()) }); $('output').textContent = JSON.stringify(result, null, 2); } catch (error) { $('output').textContent = error.message; } };
$('open').onclick = async () => { try { const names = await api('/api/recipes'); const choice = prompt('输入 recipe 文件名:\n' + names.join('\n')); if (choice) { state.recipe = await api('/api/recipe/' + encodeURIComponent(choice)); state.selected = null; draw(); } } catch (error) { $('output').textContent = error.message; } };
