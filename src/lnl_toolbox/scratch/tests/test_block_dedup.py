...
class ScratchBlockDedupTest(unittest.TestCase):
    ...
    def test_common_masked_and_weighted_cross_entropy(self) -> None:
        ...
        BLOCKS["per_sample_ce"].execute(ctx, logits="logits", labels="labels", save_as="masked_per_sample")
        BLOCKS["masked_mean"].execute(ctx, values="masked_per_sample", mask="mask", save_as="masked")
        BLOCKS["per_sample_ce"].execute(ctx, logits="logits", labels="pseudo", save_as="weighted_per_sample")
        BLOCKS["detach"].execute(ctx, input="weights", save_as="detached_weights")
        BLOCKS["elementwise_multiply"].execute(ctx, left="weighted_per_sample", right="detached_weights", save_as="weighted_per_sample")
        BLOCKS["mean_loss"].execute(ctx, input="weighted_per_sample", save_as="weighted")
