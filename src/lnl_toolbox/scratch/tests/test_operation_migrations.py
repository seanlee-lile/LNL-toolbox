...
class ScratchOperationMigrationTest(unittest.TestCase):
    ...
    def test_pdl_masked_transition_chain(self) -> None:
        ...
        run("softmax", ctx, logits="logits", detach=False, save_as="clean_probabilities")
        run("materialize_transition", ctx, artifact="transition", indices="indices", save_as="transition_matrix")
        run("apply_transition", ctx, probabilities="clean_probabilities", transition="transition_matrix", save_as="observed_probabilities")
        run("gather_by_label", ctx, values="clean_probabilities", labels="labels", save_as="clean_y")
        run("gather_by_label", ctx, values="observed_probabilities", labels="labels", save_as="observed_y")
        run("safe_divide", ctx, numerator="clean_y", denominator="observed_y", save_as="ratio")
        run("negative_log", ctx, input="clean_y", save_as="negative_log_y")
        run("elementwise_multiply", ctx, left="ratio", right="negative_log_y", save_as="risk")
