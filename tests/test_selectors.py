...
# --- merged from test_selectors.py ---
from lnl_toolbox.selectors import AllSelector, ConstantKeepRateSchedule, LinearKeepRateSchedule, SelectionInput, SelectionResult, SmallLossSelector, IndexedTensorHistory, validate_selection_result

# --- merged from test_selectors.py ---
class _selectors_SelectorTest(unittest.TestCase):

    def test_indexed_tensor_history_roundtrip_and_contract(self) -> None:
        history = IndexedTensorHistory(4, 3, 2)
        history.update(torch.tensor([3, 1]), 0, torch.tensor([[0.3, 0.7], [0.8, 0.2]]))
        ...
        with self.assertRaisesRegex(ValueError, 'unique'):
            history.update(torch.tensor([1, 1]), 1, torch.ones(2, 2))
    ...
    def test_schedule_rejects_invalid_parameters_and_epochs(self) -> None:
        for kwargs in ({'start': 0.0, 'end': 0.6, 'warmup_epochs': 10}, {'start': 1.0, 'end': 1.1, 'warmup_epochs': 10}, {'start': 1.0, 'end': float('nan'), 'warmup_epochs': 10}, {'start': 1.0, 'end': 0.6, 'warmup_epochs': 0}):
    ...
    def test_all_selector_keeps_every_sample(self) -> None:
        result = AllSelector().select(SelectionInput(scores=torch.tensor([0.8, 0.2, 0.4]), sample_indices=torch.tensor([12, 3, 9])))
    ...
    def test_small_loss_keeps_fixed_rate(self) -> None:
        result = SmallLossSelector(keep_rate=0.5).select(SelectionInput(scores=torch.tensor([0.8, 0.2, 0.4, 0.1]), sample_indices=torch.tensor([12, 3, 9, 20])))
    ...
    def test_small_loss_accepts_constant_mapping_and_missing_epoch_defaults_to_zero(self) -> None:
        ...
        missing_epoch = selector.select(SelectionInput(scores=torch.arange(4, dtype=torch.float32), sample_indices=torch.arange(4)))
        explicit_epoch_zero = selector.select(SelectionInput(scores=torch.arange(4, dtype=torch.float32), sample_indices=torch.arange(4), metadata={'epoch': 0}))
        self.assertTrue(torch.equal(missing_epoch.selected_mask, explicit_epoch_zero.selected_mask))
    ...
    def test_small_loss_linear_schedule_selects_expected_count_by_epoch(self) -> None:
        selector = SmallLossSelector({'name': 'linear', 'start': 1.0, 'end': 0.6, 'warmup_epochs': 2})
        expected = {0: (10, 1.0), 1: (8, 0.8), 2: (6, 0.6), 3: (6, 0.6)}
        for epoch, (count, rate) in expected.items():
            with self.subTest(epoch=epoch):
                result = selector.select(SelectionInput(scores=torch.arange(10, dtype=torch.float32), sample_indices=torch.arange(10), metadata={'epoch': epoch}))
    ...
    def test_small_loss_rejects_uncertain_epoch_metadata(self) -> None:
        ...
        for epoch, error in ((None, TypeError), (1.5, TypeError), (True, TypeError), (-1, ValueError)):
            with self.subTest(epoch=epoch), self.assertRaises(error):
                selector.select(SelectionInput(scores=torch.tensor([0.1, 0.2]), sample_indices=torch.tensor([1, 2]), metadata={'epoch': epoch}))

    def test_small_loss_rounds_up_and_keeps_at_least_one(self) -> None:
        result = SmallLossSelector(keep_rate=0.01).select(SelectionInput(scores=torch.tensor([0.3, 0.1, 0.2]), sample_indices=torch.tensor([1, 2, 3])))
    ...
    def test_equal_scores_use_stable_global_index_tie_break(self) -> None:
        ...
        first = selector.select(SelectionInput(scores=torch.ones(4), sample_indices=torch.tensor([40, 10, 30, 20])))
        second = selector.select(SelectionInput(scores=torch.ones(4), sample_indices=torch.tensor([20, 40, 10, 30])))
        first_ids = {value for value, keep in zip([40, 10, 30, 20], first.selected_mask.tolist()) if keep}
        second_ids = {value for value, keep in zip([20, 40, 10, 30], second.selected_mask.tolist()) if keep}
    ...
    def test_selector_rejects_scores_attached_to_autograd(self) -> None:
        with self.assertRaisesRegex(ValueError, 'detached'):
            AllSelector().select(SelectionInput(scores=torch.tensor([0.1, 0.2], requires_grad=True), sample_indices=torch.tensor([1, 2])))

    def test_selector_rejects_duplicate_indices(self) -> None:
        with self.assertRaisesRegex(ValueError, 'unique'):
            SmallLossSelector(0.5).select(SelectionInput(scores=torch.tensor([0.1, 0.2]), sample_indices=torch.tensor([1, 1])))
    ...
    def test_result_requires_boolean_aligned_nonempty_mask(self) -> None:
        with self.assertRaisesRegex(ValueError, 'torch.bool'):
            validate_selection_result(SelectionResult(selected_mask=torch.tensor([1, 0])), batch_size=2, device=torch.device('cpu'))
        with self.assertRaisesRegex(ValueError, 'at least one'):
            validate_selection_result(SelectionResult(selected_mask=torch.zeros(2, dtype=torch.bool)), batch_size=2, device=torch.device('cpu'))
