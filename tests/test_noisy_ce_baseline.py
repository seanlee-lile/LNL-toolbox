...
from lnl_toolbox.training.clean_baseline import run_clean_experiment
...
class NoisyCeBaselineTest(unittest.TestCase):
    def test_noisy_training_writes_manifest_metadata_and_clean_evaluation_sets(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory, patch(
            "lnl_toolbox.training.clean_baseline.load_cifar10", side_effect=load_data
        ), patch("lnl_toolbox.training.clean_baseline.evaluate_classification", side_effect=evaluate):
            run_dir = run_clean_experiment(_config(), directory)
        ...
        self.assertEqual(set(checkpoint["noise"]), {
            "manifest_path", "manifest_version", "mapping_hash", "dataset", "split",
    ...
    def test_non_ce_algorithm_is_rejected(self) -> None:
        config = _config()
        config["algorithm"]["name"] = "gce"
        with tempfile.TemporaryDirectory() as directory, self.assertRaisesRegex(ValueError, "Unsupported algorithm"):
            run_clean_experiment(config, directory)
