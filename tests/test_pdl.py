...
# --- merged from test_pdl.py ---
from lnl_toolbox.algorithms.instance_transition import _row_positive_normalize, pdl_instance_corrected_losses

# --- merged from test_pdl.py ---
from lnl_toolbox.algorithms.transition_risk import forward_instance_corrected_losses, instance_importance_reweighted_losses
...
# --- merged from test_pdl.py ---
from lnl_toolbox.noise import PartTransitionArtifact, PartTransitionEstimator, PosteriorSnapshot, fit_part_representation, fit_part_transition_matrices, generate_pdl_idn, select_anchor_candidates
...
class _pdl_PDLTest(unittest.TestCase):
    ...
    def test_revision_projects_negative_entries_to_zero(self) -> None:
        ...
        torch.testing.assert_close(
            actual[0, 0], torch.tensor([0.0, 1.0])
        )
        with self.assertRaisesRegex(ValueError, "all-zero"):
            _row_positive_normalize(torch.tensor([[[-0.2, -0.8]]]))
    ...
    def test_official_cifar_resnet34_stem_setting_is_available(self) -> None:
        model = cifar_resnet34(10, initialization='torch_default', stem_padding=0)
    ...
    def test_generator_matches_official_rng_and_softmax_order(self) -> None:
        ...
        flip_rate = stats.truncnorm((0.0 - 0.4) / 0.1, (1.0 - 0.4) / 0.1, loc=0.4, scale=0.1).rvs(labels.shape[0])
        weights = torch.as_tensor(np.random.randn(3, 6, 3), dtype=torch.float32)
        probabilities = []
        for position, clean_class in enumerate(labels):
            scores = torch.as_tensor(inputs[position], dtype=torch.float32).reshape(1, -1).mm(weights[int(clean_class)]).squeeze(0)
        ...
        reference_labels = np.asarray([np.random.choice([0, 1, 2], p=reference_probabilities[position]) for position in range(labels.size)])
    ...
    def test_anchor_candidates_break_ties_by_global_index(self) -> None:
        snapshot = PosteriorSnapshot(np.array([[0.8, 0.2], [0.8, 0.2], [0.1, 0.9]]), np.array([0, 0, 1]), np.array([9, 2, 7]), 'toy', 'train')
    ...
    def test_official_nmf_matches_train_m_global_rng_and_final_normalization(self) -> None:
        features = np.array([[0.9, 0.1, 0.3], [0.2, 0.8, 0.4], [0.6, 0.4, 0.7]], dtype=np.float64)
        iterations = 4
        np.random.seed(23)
        ...
        for _ in range(iterations):
            ...
            if float(np.square(error).sum()) < 1e-05:
                break
            latent *= np.divide(weights.T @ features, weights.T @ weights @ latent, out=np.zeros_like(latent), where=weights.T @ weights @ latent != 0.0)
            weights *= np.divide(features @ latent.T, weights @ latent @ latent.T, out=np.zeros_like(weights), where=weights @ latent @ latent.T != 0.0)
        weights /= weights.sum(axis=1, keepdims=True)
        np.random.seed(23)
        actual_parts, actual_coefficients = fit_part_representation(features, 2, seed=None, iterations=iterations, error_tolerance=1e-05)
    ...
    def test_official_basis_reset_matches_init_params_scale(self) -> None:
        ...
        coefficients = np.array([[[0.9, 0.1], [0.8, 0.2]], [[0.2, 0.8], [0.1, 0.9]]])
        targets = coefficients.copy()
        actual = fit_pdl_basis_matrices(coefficients, targets, epochs=2, loss_threshold=0.0, seed=29)
        ...
        for class_index in range(2):
            ...
            for _ in range(2):
                ...
                for basis_index in range(2):
                    with torch.no_grad():
                        weights.copy_(weights.abs() / weights.abs().sum(dim=1, keepdim=True).clamp_min(1e-12))
                    prediction = (torch.as_tensor(coefficients[class_index, basis_index], dtype=torch.float32)[:, None] * weights).sum(dim=0)
                    optimizer.zero_grad(set_to_none=True)
                    loss = F.mse_loss(prediction, torch.as_tensor(targets[class_index, basis_index], dtype=torch.float32))
                    loss.backward()
                    optimizer.step()
                    loss_total = loss_total + loss.detach()
            with torch.no_grad():
                expected[:, class_index, :] = (weights.abs() / weights.abs().sum(dim=1, keepdim=True).clamp_min(1e-12)).numpy()
    ...
    def test_official_anchor_and_basis_shapes(self) -> None:
        from lnl_toolbox.noise.pdl import fit_pdl_basis_matrices, select_pdl_anchor_candidates
        probabilities = np.array([[0.99, 0.01], [0.98, 0.02], [0.02, 0.98], [0.01, 0.99]])
        positions = select_pdl_anchor_candidates(probabilities, [97.0, 99.0])
        self.assertEqual(positions.shape, (2, 2))
        coefficients = np.array([[[0.9, 0.1], [0.8, 0.2]], [[0.2, 0.8], [0.1, 0.9]]])
        targets = np.array([[[0.9, 0.1], [0.8, 0.2]], [[0.2, 0.8], [0.1, 0.9]]])
        matrices = fit_pdl_basis_matrices(coefficients, targets, epochs=3, loss_threshold=0.0, seed=4)
        self.assertEqual(matrices.shape, (2, 2, 2))
        np.testing.assert_allclose(matrices.sum(axis=2), 1.0, atol=1e-06)

    def test_eq4_recovers_known_part_matrices(self) -> None:
        part_matrices = np.array([[[0.9, 0.1], [0.2, 0.8]], [[0.6, 0.4], [0.3, 0.7]]])
        coefficients = np.array([[[1.0, 0.0], [0.0, 1.0]], [[0.75, 0.25], [0.25, 0.75]]])
        posteriors = np.empty((2, 2, 2))
        for clean_class in range(2):
            posteriors[clean_class] = np.einsum('kr,rc->kc', coefficients[clean_class], part_matrices[:, clean_class])
    ...
    def _artifact(self) -> PartTransitionArtifact:
        return PartTransitionArtifact(parts=np.eye(2), coefficients=np.array([[1.0, 0.0], [0.25, 0.75]]), part_matrices=np.array([[[0.9, 0.1], [0.2, 0.8]], [[0.6, 0.4], [0.3, 0.7]]]), global_indices=np.array([11, 4]), feature_snapshot_hash='a' * 64, posterior_snapshot_hash='b' * 64, anchor_indices=np.array([[4, 11], [11, 4]]))
    ...
    def test_official_raw_basis_artifact_clips_only_combined_train_matrix(self) -> None:
        artifact = PartTransitionArtifact(parts=np.eye(2), coefficients=np.array([[1.0, 0.0]]), part_matrices=np.array([[[0.8, 0.3], [-0.1, 0.2]], [[0.2, -0.1], [0.4, 0.6]]]), global_indices=np.array([0]), feature_snapshot_hash='a' * 64, posterior_snapshot_hash='b' * 64, anchor_indices=np.array([[0], [0]]), metadata={'official_raw_basis': True})
    ...
    def test_official_split_estimation_shares_nmf_and_separates_basis_artifacts(self) -> None:
        ...
        features = FeatureSnapshot(np.array([[1.0, 0.0, 0.2], [0.8, 0.2, 0.1], [0.1, 0.9, 0.2], [0.2, 0.8, 0.1], [0.6, 0.4, 0.3], [0.3, 0.7, 0.4]]), np.array([0, 0, 1, 1, 0, 1]), indices, 'toy', 'union')
        posteriors = PosteriorSnapshot(np.array([[0.9, 0.1], [0.75, 0.25], [0.15, 0.85], [0.25, 0.75], [0.65, 0.35], [0.35, 0.65]]), features.noisy_targets, indices, 'toy', 'union')
        estimator = PartTransitionEstimator(num_parts=2, anchor_candidates=2, representation_seed=7, representation_iterations=2, anchor_percentages=(97.0, 99.0), basis_epochs=1, basis_loss_threshold=0.0)
        parts, coefficients = fit_part_representation(features.features, 2, seed=7, iterations=2)
        train = FeatureSnapshot(features.features[:3], features.noisy_targets[:3], indices[:3], 'toy', 'train')
        train_posterior = PosteriorSnapshot(posteriors.noisy_probabilities[:3], posteriors.noisy_targets[:3], indices[:3], 'toy', 'train')
        validation = FeatureSnapshot(features.features[3:], features.noisy_targets[3:], indices[3:], 'toy', 'validation')
        validation_posterior = PosteriorSnapshot(posteriors.noisy_probabilities[3:], posteriors.noisy_targets[3:], indices[3:], 'toy', 'validation')
        train_artifact = estimator.estimate_from_shared_representation(train, train_posterior, representation_parts=parts, representation_coefficients=coefficients, representation_indices=indices)
        validation_artifact = estimator.estimate_from_shared_representation(validation, validation_posterior, representation_parts=parts, representation_coefficients=coefficients, representation_indices=indices)
        ...
        revision_artifact = validation_artifact.with_part_matrices(train_artifact.part_matrices, role='revision_validation', source_artifact_hash=train_artifact.artifact_hash)
        np.testing.assert_allclose(revision_artifact.parts, validation_artifact.parts)
        np.testing.assert_allclose(revision_artifact.coefficients, validation_artifact.coefficients)
        np.testing.assert_allclose(revision_artifact.part_matrices, train_artifact.part_matrices)
        self.assertEqual(revision_artifact.metadata['artifact_role'], 'revision_validation')
        self.assertEqual(revision_artifact.metadata['source_artifact_hash'], train_artifact.artifact_hash)
    ...
    def test_forward_and_importance_objectives_match_manual_values(self) -> None:
        ...
        matrices = torch.tensor([[[0.8, 0.2], [0.1, 0.9]], [[1.0, 0.0], [0.0, 1.0]]], dtype=torch.float64)
        ...
        expected_weight = (clean.gather(1, targets[:, None]).squeeze(1) / noisy.gather(1, targets[:, None]).squeeze(1)).detach()
    ...
    def test_pdl_reweight_objective_matches_official_beta_nll(self) -> None:
        ...
        official_noisy = torch.bmm(official_clean.unsqueeze(1), matrices).squeeze(1)
        ...
        pdl_instance_corrected_losses(correction_logits, targets, matrices, detach_importance_weight=True).mean().backward()
        ...
        pdl_instance_corrected_losses(revision_logits, targets, matrices, detach_importance_weight=False).mean().backward()
    ...
    def test_pdl_algorithm_uses_phase_specific_beta_gradients(self) -> None:
        batch = Batch({'input': torch.tensor([[1.0, 0.0], [0.0, 1.0]]), 'target': torch.tensor([0, 1]), 'index': torch.tensor([11, 4])})
        artifact = self._artifact()
        for correction, expected_detach in (('pdl', True), ('pdl_revision', False)):
            ...
            algorithm = InstanceTransitionClassificationAlgorithm(model, torch.optim.SGD(model.parameters(), lr=0.0), nn.CrossEntropyLoss(reduction='none'), artifact, torch.device('cpu'), correction=correction)
            algorithm.setup(ExperimentContext(work_dir=Path('.')))
            with patch('lnl_toolbox.algorithms.instance_transition.pdl_instance_corrected_losses', wraps=pdl_instance_corrected_losses) as corrected_loss:
                algorithm.step(batch, RunState())
            self.assertEqual(corrected_loss.call_args.kwargs['detach_importance_weight'], expected_detach)

    def test_algorithm_checkpoint_rejects_artifact_change(self) -> None:
        artifact = self._artifact()
        model = nn.Linear(2, 2)
        optimizer = torch.optim.SGD(model.parameters(), lr=0.1)
        algorithm = InstanceTransitionClassificationAlgorithm(model, optimizer, nn.CrossEntropyLoss(reduction='none'), artifact, torch.device('cpu'), correction='forward')
        algorithm.setup(ExperimentContext(work_dir=Path('.')))
        state = RunState()
        algorithm.on_cycle_start(state)
        result = algorithm.step(Batch({'input': torch.tensor([[1.0, 0.0], [0.0, 1.0]]), 'target': torch.tensor([0, 1]), 'index': torch.tensor([11, 4])}), state)
